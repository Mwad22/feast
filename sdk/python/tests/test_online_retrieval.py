...
def test_online() -> None:
    ...
    with runner.local_repo(
        ...
        # Ensure setting feature_names_only to true strips featureview prefixes
        # from feature names
        result = store.get_online_features(
            feature_refs=[
                "driver_locations:lon",
        ...
        with pytest.raises(FeatureViewNotFoundException):
            store.get_online_features(
                feature_refs=["driver_locations_bad:lon"], entity_rows=[{"driver": 1}],feature_names_only=False
            )
